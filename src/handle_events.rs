...
pub fn handle_events<'a>(window: &mut RenderWindow, view: &mut View, person: &mut Person<'a>) {
	for event in window.events() {
		...
		macro_rules! sv {
			($l:expr, $r:expr) => ( person.speed.1 += $r * 0.7; person.speed.0 += $l * 0.4; view.set_center2f(person.person.get_position().x, person.person.get_position().y); window.set_view(&view) );
